...
| F0.6 | El PDF combinado se devuelve con `tipo: combinado` y campo `partes`, **y además** sus partes; `documentos_legibles()` entrega solo las partes para consolidar. Las partes conservan la numeración de página del original y el `sha256` del fichero entregado | Una cita debe apuntar a la página real del fichero que entregó el cliente | Devolver solo las partes (se pierde el original) |
| F0.6 | Corte de un combinado: página cuyo **título** (línea de mayor cuerpo, `upright`) clasifica con confianza ≥ 0,6; solo se separa con dos cortes o más. Giro de un escaneo: gana la rotación con más palabras reconocibles según el TSV de tesseract | La marca de agua girada queda fuera por `upright`; un convenio de dos páginas no se parte | Cortar por página en blanco; heurística de vocales |
| F0.6 | Extensión del contrato §2.2: `Pagina.texto_ocr` (OCR de una página que además tiene texto nativo, para la placa dentro del informe fotográfico), `Documento.avisos` y `Documento.partes`; los avisos de ingesta no van en `ActuacionConsolidada`, los concatena `motor.py` con `avisos_de(docs)` | Sin `texto_ocr` la placa se pierde o se mezcla con el texto nativo | Duplicar la página como documento OCR |
| F0.7 | **La spec manda sobre el léxico**: un campo solo se emite si el tipo de documento figura en `fuentes`/`cruce_con`/`derivacion.fuente` de esa variable; dos pasadas (todas las tablas y después el texto); la categoría de una línea de factura se decide por su cabeza, no por la descripción entera | Reproduce el ground truth sin ninguna rama por ficha y respeta la trampa "motor existente" | Emitir todo y filtrar al consolidar |
| F0.7 | El registro deriva lo que la spec declara y **como** lo dice su prosa, resuelta contra un catálogo de métodos por palabras clave (igual que el criterio de redondeo en `calculo.py`); método no reconocido → aviso, nunca un criterio inventado; el periodo cuenta el último intervalo (36 días exactos en A) | Reglas de oro 4 y 7 | Cablear N2, P_prom y h_despues |
...
6. **`n_motores` contado desde el registro y el informe fotográfico** (series distintas): si en una actuación real
   falta la documentación de un motor, fallaría `R-CON-07` (`BLOQUEANTE_DATOS` → `BLOQUEADO`) en vez de `R-DOC-01`
   (`SUBSANABLE`). Conviene revisar si ese es el comportamiento deseado o si `R-CON-07` debería ser subsanable.
