...
| **`GAP-HTTP-03`** · Empaquetador y página que monten las superficies de `front/` | El servidor ya publica el contrato, pero `front/` se consume como código fuente y no hay nada que abrir en un navegador | La promesa de «abrir la cola de revisión» sigue sin cumplirse, y ya no depende del backend | `ADR-015` §7.2, `FR-HTTP` 24/09/2026 |
