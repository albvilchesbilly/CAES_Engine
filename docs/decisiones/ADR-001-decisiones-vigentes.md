...
| **Proveedor de identidad y mapeo de sus grupos a los ocho perfiles** | `FR-HTTP` dejó el puerto `Autenticador` (C28) y una implementación de desarrollo que solo sirve en local. **El mapeo es la decisión de verdad**, y necesita un cliente delante | Sin esto no hay sesión real: hoy el principal lo declara una cabecera | `ADR-015` §4, `FR-HTTP` 24/09/2026 |
| **`FR-DESPLIEGUE`** · Dónde vive el sistema (TLS, dominio, configuración, procedimiento) | Ligado a `API-09`: si el certificado del perfil Modificación puede usarse desde infraestructura de un tercero | Sin esto no hay producción, por mucho servidor que haya | `ADR-015` §5, `FR-HTTP` 24/09/2026 |
| **Límites de tamaño y de frecuencia** | `FR-HTTP` puso 8 MiB de cuerpo **como valor provisional, no medido**, y **ningún límite de peticiones** | Un servidor que sirve documentos acepta subidas; es decisión de producto | `ADR-015` §6 y §7 |
| **`GAP-HTTP-02`** · Cómo viaja el contenido de un documento que se sube (`CAP-02`) | El manejador exige `bytes` y el sobre es JSON. El simétrico de `documento_a_transporte` falta **en `api/`**, no en el servidor: ponerlo en la capa HTTP sería decidir allí una codificación que el contrato no declara | **`FR2` no puede cerrar su pantalla sin esto**: la subida es su corazón | `ADR-015` §7.1, `FR-HTTP` 24/09/2026 |
| **`GAP-HTTP-04`** · La cola de la demo no tiene antigüedades ni estado de ciclo | `servidor_desarrollo.py` mete los siete casos en el repositorio **sin log**: `antiguedad.abierta_en` llega `null`, la secuencia es 0 y todas las filas dicen `ESPERANDO DESDE · SIN DATO` y `Ciclo: ABIERTA`. La pantalla se porta bien (no inventa el dato); es la composición la que no tiene historia que contar. `generar.py` sí puebla el log, por eso los tests de `FR1` no lo enseñan | Una columna entera de la cola vacía en la primera demo. Rellenarla es **decidir qué historia tienen los casos sintéticos**, no un detalle técnico | `GAP-HTTP-03`, 24/09/2026, `ADR-015` §7.3 |
| **El motivo de «Subir documentación» se ha quedado viejo** | Dice «necesita la capa HTTP, que todavía no existe (`GAP-HTTP-01`)». La capa HTTP existe desde el 24/09; lo que impide subir es `GAP-HTTP-02`. El fondo es cierto —no se puede subir— pero la razón que se enseña es falsa | Un motivo inhabilitado que miente es peor que no dar motivo. Lo arregla `FR2` al cerrar esa pantalla; no se tocó al montar el front para no mover las pantallas de `FR1` | `GAP-HTTP-03`, 24/09/2026, `ADR-015` §7.3 |
| **`CAP-01` (abrir actuación) no se puede ejercer por la puerta de `api/`** | Nombrar la actuación nueva la deniega `comprobar_alcance` —falla cerrado y no puede decir de qué tenant es algo que todavía no existe— y no nombrarla la deniega `Peticion.actuacion_id`. Verificado por las tres vías el 24/09/2026. El test que había (`test_abrir_una_actuacion_nueva_no_pasa_por_el_alcance`) llama a `comprobar_alcance` suelto, no a `ejecutar`, así que no lo veía | **Bloquea el alta de `FR2`**: no hay forma de dar de alta una actuación por el contrato. Decisión: o el alta se exceptúa del alcance de forma declarada (la capacidad la crea, no la consulta), o el repositorio distingue «no existe» de «no sé de quién es» | `FR-HTTP`, 24/09/2026 |
