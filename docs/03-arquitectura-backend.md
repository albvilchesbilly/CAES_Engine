...
| N4 | Evidence Store | Grafo de evidencias, tres capas por dato, consolidación, conflictos, `tratable_por_plataforma`; **la corrección humana es una evidencia más** (S3.8, `ADR-013`) | `engine/evidencias.py`, `engine/correcciones.py` | `EXISTE` (F0.8, ampliado en S3.8) en memoria con serialización JSON (§14 e) · `NUEVO` S3: persistencia junto al log |
...
| S4 | Consola de revisión | Escalados, conflictos, correcciones del profesional, tareas pendientes de la plataforma; centrada en lo previo a la firma. Con `ADR-006`, es donde viven las capacidades `CAP-nn` de los perfiles de tenant | `front/workspace/` (cola y vista de revisión) sobre `api/` | `EXISTE` (`FR1`, 23/09/2026, `ADR-014`; absorbió `S4.4`). **Se abre en un navegador desde el 24/09/2026** contra datos sintéticos: `FR-HTTP` publica `api/` por red y `front/aplicacion/` lo monta (`GAP-HTTP-03` cerrado, `ADR-015` §7.2) |
...
| F-14 | Consumo de notificaciones y **tareas pendientes** de la plataforma; cola priorizada por tenant | CAE Platform | P9 / S4 | Sí | `PARCIAL`: `EXISTE` el consumo desde el simulador (S3.4 y S3.5) y la cola priorizada (`FR1`, `CAP-17`: el servidor ordena, la pantalla no) |
...
| c | Qué regla iba en qué fase | **La asignación de reglas a fases es la tabla de fases de `docs/04`**. Si el YAML declara `fase`, se respeta; si no (la v1.1 no la declara), el Spec Registry la **deriva** de la severidad y de lo que referencia la `logica` (`docs/04` §3.2), sin lista de reglas por fase en el código |
