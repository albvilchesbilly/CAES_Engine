...
| `CAP-03` | `calculo` | `total_exacto`, `total_cae`, sus dos `*_presentable`, `total_unidad` (la unidad que declara la ficha, de `spec.calculo.total.unidad`), `provisional`, `motivo_no_calculo`, `traza[]`, `variables{}` (la descripción de cada entrada y derivada, de la spec) y `por_unidad[]` (`entradas`, `derivadas`, sus `*_presentables`, `fuentes`, `salida`, `salida_presentable`, `salida_unidad`, `controles`, `precondiciones`, `interpretaciones`, `avisos`, `motivo_no_calculo`) |
