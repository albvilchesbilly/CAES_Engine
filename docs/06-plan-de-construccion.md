...
| FR1 | Workspace `T-REV`: cola y vista de revisión. **Absorbe `S4.4`** | FR0 (HECHO), S3.5 (HECHO), **S3.8 (HECHO)**: el recálculo ya existe; falta que `api/` lo dispare | EN CURSO: contrato y specs hechos; pantallas pendientes |
...
| **`R-AMB-02` no dispara con una factura real** (`/contrastar` 20/09/2026, **verificado**): `engine.extraccion.categoria_de_linea` clasifica «Suministro e instalación de motor nuevo 110 kW» y «Montaje de bomba centrífuga nueva BCN-250» como `instalacion`, porque decide por la cabeza de la descripción y la señal `instalacion` va por delante de `motor` y `bomba` | **Una actuación que la ficha excluye (EXC-01, EXC-02) sale `PREVALIDADA` con ahorro publicado.** `R-AMB-02` es `BLOQUEANTE_AMBITO` y su mitad negativa solo se ejercita en contextos fabricados en memoria: ni el banco A–G ni las metamórficas lo notarían | Hallazgo abierto para `engine/extraccion.py`: reordenar las señales y añadir las de compra (`suministro de motor`, `motor nuevo`, `sustitución`), **más un caso de banco o una metamórfica con factura de motor nuevo**, que es lo que impide que vuelva |
