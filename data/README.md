...
- **`<nombre>.csv`**: separador coma, punto decimal, sin separador de miles, **sin notación exponencial** (`1e3` es error de carga), UTF-8, cabecera en la primera línea que debe coincidir con `columnas` del meta (mismo conjunto y mismo orden). Los valores se leen como `Decimal(texto)`; nunca pasan por `float`; una celda vacía es `null`; `NaN`, `sNaN` e `Infinity` son error de carga. Última columna siempre `verificado` ∈ {`si`, `pendiente`}; cualquier otro valor es error de carga. Las filas van en **orden estrictamente creciente por `clave`** tal como se transcriben: el motor no reordena ni admite claves repetidas (sería arreglar en silencio una transcripción).
- **`<nombre>.meta.yaml`**: `id` (el que usa la spec en `tablas.<ID>`), `fuente` (URL oficial), `fecha_transcripcion`, `transcrito_por`, `metodo_transcripcion` (`doue` | `boe` | `memoria_agente`), `verificacion` (responsable, filas verificadas, alcance), `vigencia: {desde, hasta}` (ISO `AAAA-MM-DD` sin hora; `hasta: null` si sigue vigente), `columnas` (todas, incluida `verificado`), `unidades`, `clave` (columna por la que se busca), `valor` (columna que devuelve `buscar()` e interpola INT-02) y, opcionalmente, `restricciones` (hoy solo `positivo: [columnas]`: esas columnas deben ser > 0 en toda fila; una tabla futura con coeficientes negativos simplemente no la declara). `clave` y `valor` son obligatorios, distintos entre sí y numéricos; `engine/tablas.py` no conoce ninguna columna por su nombre: **el esquema de cada tabla es este meta** (regla de oro 4). Una restricción desconocida es error de carga.

El motor busca por `clave` exacta y devuelve la columna `valor`. Si no hay fila exacta, aplica el criterio **INT-02** de la spec (interpolación lineal de `valor` entre las dos filas adyacentes, `Decimal` sin redondeo) y devuelve la marca `INT-02` con aviso para el informe. Fuera del rango de la tabla **no extrapola**: devuelve `valor = null` y aviso. Si alguna fila usada está `pendiente`, el aviso lo dice ("tabla pendiente de verificación humana"). Buscar con un valor no finito es error (`ErrorTabla`), nunca un resultado.

Sanidad en carga (error = la tabla no se carga): `clave` y `valor` sin celdas vacías, `clave` estrictamente creciente, columnas de `restricciones.positivo` > 0, celdas numéricas finitas y sin exponente, cabecera igual a `columnas`, tabla con al menos una fila. Si `valor` **decrece** entre dos claves consecutivas no es error (una tabla real puede hacerlo) pero queda registrado en `Tabla.avisos_carga` para la revisión humana; en el cuadro 6 transcrito no hay ninguno.
...
|---|---|---|---|---|---|
| cuadro 6 | `kw_motor = 110` | `perdidas_ref_kw = 5.55` | Billy / Sprint 1 del Engine 0.1 | 17/09/2026 | BOE/DOUE, **según este README en la consolidación documental (18/09/2026) y `CLAUDE.md` §5**. `docs/historico/cae-engine-estado-proyecto_2026-09-17.md` §4 solo registra 5,55 como valor del cuadro 6 en el Engine 0.1 ("verificada en BOE/DOUE", sin quién, cuándo ni contra qué texto): **no es un acta del contraste**. Ver advertencia 5 y ADR-002 §6.1 |
...
5. **Observación de la revisión QA de F0.2 (18/09/2026) sobre la fila 110 kW.** En la transcripción actual el cociente `perdidas_ref_kw / kw_motor` describe una serie suave y decreciente: 5,90 % (30 kW), 5,84 % (37), 5,78 % (45), 5,67 % (55), 5,60 % (75), 5,59 % (90), **5,05 % (110)**, 5,55 % (132), 5,51 % (160), 5,50 % (200), 5,48 % (250). La fila de 110 kW (5,55 kW) es la única que se sale de la serie; el valor que recordaba el agente, 6,11 kW, daría 5,55 %, coherente con las vecinas. Esto **no demuestra** que 5,55 sea erróneo (las 38 filas vecinas son de memoria y sin verificar; podrían ser ellas las desviadas), pero sí que la fila 110 y su serie no pueden ser ambas correctas tal como están. La fila se mantiene `verificado: si` por precedencia documental (este README y `CLAUDE.md` §5 sobre el histórico) y **la decisión es de Billy** (ADR-002 §6.1): confirmar 5,55 contra el DOUE, o corregir a 6,11 (entonces cambian INT-01, el caso A y el criterio 305.829,6 kWh/año: ADR nuevo). El CSV no se toca hasta entonces.
